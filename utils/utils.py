...
class losses_saver():
    def __init__(self, opt):
        self.name_list = ["Generator", "Vgg", "D_fake", "D_real"]
        self.opt = opt
...
class image_saver():
    ...
    def visualize_batch(self, model, real, rendered, cur_iter):
        self.save_images(rendered, "rendered", cur_iter)
        self.save_images(real, "real", cur_iter)
        with torch.no_grad():
            model.eval()
            fake = model.module.netG(rendered)
    ...
    def save_images(self, batch, name, cur_iter):
        fig = plt.figure()
