...
dataloader, dataloader_val, _ = dataloaders.get_dataloaders(opt)
...
for epoch in range(start_epoch, opt.num_epochs):
    for i, data_i in enumerate(dataloader):
        ...
        loss_D, losses_D_list = model(real, rendered, mask, "losses_D", losses_computer)
        ...
        if cur_iter % opt.freq_print == 0:
            im_saver.visualize_batch(model, real, rendered, cur_iter)
